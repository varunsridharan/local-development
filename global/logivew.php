...
$pelv_settings = file_exists( '../../' . $pelv_path ) ? parse_ini_file( '../../' . $pelv_path ) : $log_view_config;
...
class Pelv_Log_Handler {
	...
	public $default_settings = array(
		'file_path'           => 'debug.log',
		'vscode_links'        => true,
		// Stack trace references files. link them to your repo (https://code.visualstudio.com/docs/editor/command-line#_opening-vs-code-with-urls).
		'vscode_path_search'  => '',
		// This is needed if you develop on a vm. like '/srv/www/...'.
		'vscode_path_replace' => '',
		// The local path to your repo. like 'c:/users/...'.
		// 'password'            => '',
	);
	...
	public function link_vscode_files( $string ) {
		$string = preg_replace_callback( ' $([A-Z]:)?([\\\/][^:\(]+)[:\(]([0-9]+)\)?$', array(
			$this,
			'vscode_link_filter',
		), $string );
		$string = preg_replace_callback( ' $([A-Z]:)?([\\\/][^:\(]+) on line ([0-9]+)\)?$', array(
			$this,
			'vscode_link_filter',
		), $string );
		return $string;
	...
	public function replace_callback( $arr ) {
		...
		$this->content[ $err_id ]['cls'] = implode( ' ', array_slice( str_word_count( $this->content[ $err_id ]['msg'], 2 ), 1, 2 ) ); // the first few words of the message become class items.
...
<div id="app">
	...
	<md-table v-model="searched" md-sort="cnt" md-sort-order="asc" md-card md-fixed-header>
		<md-chip v-if="filesize">{{ readableFilesize() }}
			<!--needed for the inner filesize container to update--></md-chip>
		<md-table-toolbar>
			<h1 class="md-title">Debug.log
		...
		<md-table-row slot="md-table-row" slot-scope="{ item }">
			...
			<md-table-cell :error="item.cls" style="min-width:140px" md-label="Time" md-sort-by="t">{{ item.time }}
			</md-table-cell>
...
<script>
	...
	var app = new Vue( {
		...
		mounted() {
			this.update( this );
			var self    = this;
			var timeout = setInterval( this.update, 2000, this );
		...
		methods: {
			...
			getLog( comp ) {
				axios.get( '?debuglog&get_log' ).then( response => ( comp.handleErrors( response ) ) )
			},
			update( comp ) {
			...
			deleteLog() {
				this.searched = [];
				this.filesize = 0;
				axios.get( '?debuglog&delete_log' ).then( response => ( this.delete.data = response ) )
			}
		}
	} )
</script>
<style>
	...
	.md-table-cell[error*="Fatal"], .md-table-cell[error*="error"] {
